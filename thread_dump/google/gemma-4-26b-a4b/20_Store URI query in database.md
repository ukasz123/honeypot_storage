...
Modify [@mod.rs](file:///Users/ukasz.apps/projects/private/rust/honeypot_storage/src/db/mod.rs) and store query part of the request's uri in column `query` in `requests` table in both feature implementations: sqlite and duckdb

## Assistant

I will modify `src/db/mod.rs` to extract the query string from the request URI and include it in the `requests` table insertion for both SQLite and DuckDB implementations.

```rust
// I'll start by updating the SQLite implementation to extract and save the query string.
```
...
I have modified `src/db/mod.rs` to extract the query string from the request URI and store it in the `query` column of the `requests` table for both SQLite and DuckDB implementations.

Specifically, I made the following changes:
- In both `SqliteStorage::save_request` and `DuckdbStorage::save_request`, I added code to extract the query part using `req.uri().query().map(|q| q.to_string())`.
- Updated the `INSERT` statements in both implementations to include the new `query` column and bind the extracted value.

```rust
// src/db/mod.rs (snippet for SqliteStorage)
            let method = req.method().to_string();
            let path = req.uri().path().to_string();
            let query = req.uri().query().map(|q| q.to_string());
...
```rust
// src/db/mod.rs (snippet for DuckdbStorage)
            let method = req.method().to_string();
            let path = req.uri().path().to_string();
            let query = req.uri().query().map(|q| q.to_string());
